...
function main() {
  ...
  if is_windows; then
    from=$(cygpath -w ${PIP_FILE_PREFIX}tf_convex_quad_iou)
    to=$(cygpath -w "${TMPDIR}"/tf_convex_quad_iou)
    start robocopy //S "${from}" "${to}" //xf *_test.py
    sleep 5
  else
    rsync -avm -L --exclude='*_test.py' ${PIP_FILE_PREFIX}tf_convex_quad_iou "${TMPDIR}"
  fi
